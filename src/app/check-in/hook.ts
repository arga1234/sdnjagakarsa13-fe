...
export function useCheckInHook() {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const fields = Array.from(formData.entries()).map(([key, value]) => ({key, value}));
    const body = fields.reduce<Record<string, unknown>>((acc, {key, value}) => {
      acc[key] = value;
      return acc;
    }, {});
    ...
    try {
      ...
      router.push(`/check-in/qr-code?eventId=${x}&nama=${body.nama}&nik=${body.nik}&kelas=${kelas[body.id_kelas as string]}`);
    } catch (err) {
      console.error(err);
